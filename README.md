...
- 上記フィルタは、展示会・グッズ販売・観光PR・ゆるキャラ化などの「怪異を使った
  催し物」や、Wikipediaの用語解説・一覧ページ(「陰謀論」「未確認動物学」など、
  具体的なエピソードを含まない辞書的記事)も除外対象に含みます。
- 複数メディアが同じニュースを報じている場合、タイトルの先頭部分が一致する記事は
  `lib/sources.js`の`dedupeSimilarTitles()`により1件に統合されます(完全一致では
  なく簡易的なシグネチャ照合のため、まれに見逃し・誤統合が起きることがあります)。
